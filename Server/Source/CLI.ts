import { createInterface, ReadLine } from "readline";
import { existsSync, mkdirSync, readdirSync, copyFileSync, createWriteStream } from "fs";
import { exec, spawn, ChildProcess } from "child_process";
import { resolve } from "path";
import { inspect } from "util";
import HTTP2Server from "./Server";

const ShellPath = (...Paths: string[]) => `"${resolve(...Paths)}"`;

class CLIController {
  
  CLInterface: ReadLine;
  Controller = new Controller();

  Services: Record<string, {TypescriptCompiler: ChildProcess, Server: HTTP2Server}> = {};  

  LastPort = 17474;

  //States
  Running = false;
  CurrentProject: string;

  Ask(Question: string): Promise<string>{
    return new Promise(R => this.CLInterface.question(`${Question}: `, R));
  }

  Prompt(): Promise<string>{
    return new Promise(R => {
      this.CLInterface.once("line", R);
      this.CLInterface.prompt();
    });
  }

  Write(Data: any){
    const Type = typeof Data;
    switch(Type){
      case "string": 
        this.CLInterface.write("> " + Data);
        break;
      case "number":
      case "boolean":
      case "symbol":
      case "function":
        this.CLInterface.write("> " + Data.toString());
        break;
      case "undefined":
        this.CLInterface.write("> undefined");
        break;
      default:
        this.CLInterface.write("> " + inspect(Data));
        break;
    }
  }

  WriteLn(Data: any){
    this.Write(Data);
    this.CLInterface.write("\n");
  }

  ExecB(Command: string, Grep?: string){
    if(Grep) Command += ` | grep ${Grep}`
    return new Promise((R, E) => {
      exec(Command, (Error, StdOut, StdErr)=> {
        if(Error) E(`${Error}: ${StdErr}`);
        else R(StdOut);
      })
    })
  }

  async Confirm(Question: string): Promise<boolean>{
    for(;;){
      const Answer = (await this.Ask(`${Question} [yes|no]>`)).toLowerCase();
      if(Answer == "yes") return true;
      else if(Answer == "no") return false;
      CLI.WriteLn("Not a valid answer!");
    }
  }
  
  async Run(){
    await this.Init();
    this.CLInterface = createInterface({ 
      input: process.stdin,
      output: process.stdout,
      prompt: "Empati# "
    });
    this.Running = true;
    for(;this.Running;){
      const Raw = await this.Prompt();
      if(Raw == "" || Raw.startsWith("> ")) continue;
      const Command = Raw.trim().split(" ");
      const Fn = Command.shift();
      if(Fn in this.Controller) this.Controller[Fn].apply(this.Controller, Command);
      else {
        const AlternateFn = Fn.toLowerCase();
        if(AlternateFn in this.Controller) CLI.WriteLn(`Did you mean "${AlternateFn}"?`);
        else CLI.WriteLn("Couldn't find function!");
      }
    }
    this.CLInterface.close();
    process.exit(0);
  }

  async Init(){
    this.MakeSureDir("Projects");
    this.MakeSureDir("Backups");
    this.MakeSureDir("Logs");
    this.MakeSureDir("Configs")
  }

  MakeSureDir(Path: string){
    if(!existsSync(Path))
      mkdirSync(Path);
  }
}

class Controller {
  
  [x:string]: Function;

  ls(){
    const Dir = readdirSync("Projects");
    CLI.WriteLn(Dir);
  }

  async delete(Name?: string, Force = false){
    if(!Name && !(Name = CLI.CurrentProject)) return CLI.WriteLn("No project selected!");
    const Reset = Force || await CLI.Confirm("Do you really wanna delete this project?");
    if(!Reset) return CLI.WriteLn("Aborted.");
    CLI.WriteLn(await CLI.ExecB(`rm -rf ${resolve(`Projects/${Name}`)}`));
    CLI.WriteLn(await CLI.ExecB(`rm -rf ${resolve(`Configs/${Name}`)}`));
    CLI.WriteLn(await CLI.ExecB(`rm -rf ${resolve(`Logs/${Name}`)}`));
    this.root();
  }

  root(){
    CLI.CurrentProject = undefined;
    CLI.CLInterface.setPrompt(`Empati# `);
  }

  async init(Name?: string){
    if(!Name) Name = await CLI.Ask("Project Name");
    const Dir = resolve("Projects", Name);
    const Exists = existsSync(Dir);
    if(Exists){
      const Reset = await CLI.Confirm("Project already exists. Do you wanna reset it?");
      if(!Reset) return CLI.WriteLn("Project already exists!");
      await this.delete(Name, true);
    }
    CLI.MakeSureDir(resolve("Logs", Name));
    CLI.MakeSureDir(resolve("Configs", Name));
    mkdirSync(Dir);
    mkdirSync(resolve(Dir, "Source"));
    copyFileSync("Client/tsconfig.json", resolve(Dir, "tsconfig.json"));
    await CLI.ExecB(`ln -s ${ShellPath("Client", "Source")} ${ShellPath("Projects", Name, "Source", "Lib")}`);
    await CLI.ExecB(`openssl req -x509 -newkey rsa:2048 -nodes -sha256 -subj '/CN=localhost' -keyout ${ShellPath("Configs", 
      Name, "CertPrivate.key")} -out ${ShellPath("Configs", Name, "Cert.pem")}`);
    CLI.WriteLn(`Project initialized: ${Name}`);
  }

  async select(Name?: string){
    if(!Name) Name = await CLI.Ask("Project Name");
    const Exists = existsSync(`Projects/${Name}`);
    if(!Exists){
      const New = await CLI.Confirm("Project couldn't be found. Do you wanna create it?");
      if(New) await this.init(Name);
    }
    CLI.CurrentProject = Name;
    CLI.CLInterface.setPrompt(`${CLI.CurrentProject}$ `);
    CLI.WriteLn(`Current project: ${Name}.`);
  }

  async create(Name?: string){
    if(!Name) Name = await CLI.Ask("Project Name");
    const Exists = existsSync(`Projects/${Name}`);
    if(Exists){
      const Reset = await CLI.Confirm("Project already exists. Do you wanna backup old one and reset it?");
      if(Reset) {
        await this.backup(Name);
        await this.delete(Name, true);
        await this.init(Name);
      }
    }
    else await this.init(Name);
    CLI.WriteLn(`Project created: ${Name}`);
  }

  async run(Name?: string){
    if(!Name && !(Name = CLI.CurrentProject)) return CLI.WriteLn("No project selected!");
    const TypescriptCompiler = spawn("tsc", [], {
      cwd: resolve("Projects", Name)
    });
    const Server = new HTTP2Server(Name);
    const Port = CLI.LastPort++;
    Server.Server.listen(Port);
    CLI.Services[Name] = {TypescriptCompiler, Server};
    TypescriptCompiler.stdout.pipe(createWriteStream(resolve("Logs", Name, `tsc-stdout`)));
    TypescriptCompiler.stderr.pipe(createWriteStream(resolve("Logs", Name, `tsc-stderr`)));
    CLI.WriteLn(`Project running: ${Name} @localhost:${Port}`);
  }

  async kill(Name?: string){
    if(!Name && !(Name = CLI.CurrentProject)) return CLI.WriteLn("No project selected!");
    if(!(Name in CLI.Services)) return CLI.WriteLn(`${Name} is not running!`);
    const Service = CLI.Services[Name];
    Service.Server.Server.close();
    Service.TypescriptCompiler.kill("SIGKILL");
    delete CLI.Services[Name];
    CLI.WriteLn(`Project killed: ${Name}`);
  }

  async edit(Name?: string){
    if(!Name && !(Name = CLI.CurrentProject)) return CLI.WriteLn("No project selected!");
    const Editor = spawn(`code`, [ShellPath("Projects", Name)], {
      shell: true
    });
  }

  async backup(Name?: string){
    if(!Name && !(Name = CLI.CurrentProject)) return CLI.WriteLn("No project selected!");
    await CLI.ExecB(`cp -a ${ShellPath("Projects", Name, ".")} ${ShellPath("Backups", Name + Date.now())}`);
  }

  exit(){
    CLI.Running = false;
  }

  ps(){
    CLI.WriteLn(Object.keys(CLI.Services));
  }
}

const CLI = new CLIController();
CLI.Run();